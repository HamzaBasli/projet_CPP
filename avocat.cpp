...
{ this->id=id ; this->cin=cin ;this->nbrcas=nbrcas;
...

 {
     ...
     QString nbrcas_string=QString::number(nbrcas);
           ...
           query.prepare("INSERT INTO  AVOCAT (id, nom ,cin, prenom, nbrcas , typecas) "
...
QSqlQueryModel * Avocat ::tri_identifiant ()
    ...
    QSqlQueryModel * model= new QSqlQueryModel();
        ...
        model->setQuery("select * from AVOCAT order by id");
...
QSqlQueryModel* Avocat ::RechercheR (QString recherche)
 {
     QSqlQueryModel * model= new QSqlQueryModel();
     model->setQuery("SELECT * FROM AVOCAT WHERE id LIKE '"+recherche+"%' OR cin LIKE '"+recherche+"%' OR nom LIKE '"+recherche+"%'");
     model->setHeaderData(0, Qt::Horizontal, QObject::tr("id"));
     model->setHeaderData(1, Qt::Horizontal, QObject::tr("nom"));
     model->setHeaderData(2, Qt::Horizontal, QObject::tr("cin"));
     model->setHeaderData(3, Qt::Horizontal, QObject::tr("prenom"));
